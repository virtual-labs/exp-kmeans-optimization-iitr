...
function addDataPoint() {
    ...
    if (xValue == '+' || xValue == '-' || xValue == '*' || xValue == '/' || yValue == '+' || yValue == '-' || yValue == '*' || yValue == '/') {
        alert1('error', 'Invalid Values', 'Please enter a valid value.')
    ...
    if (isNaN(xValue)) {
        alert1('error', 'Missing Value!', 'Please enter a value for X.');
        return;
    }
    if (isNaN(yValue)) {
        alert1('error', 'Missing Value!', 'Please enter a values for Y.');
        return;
    }
...
function promptToChangeCentroids() {
    ...
    Swal.fire({
        ...
        text: "Do you want to change the selected centroid?",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
        ...
        if (result.isConfirmed) {
            ...
            alert1('info', 'Find euclidean distance!', 'To calculate the euclidean distance between the centroid and every data point.<br>Click on the <b style="color:#FF6600">DISTANCE</b> button.');
...
function updateDistanceColumn(kVal) {
    ...
    // Get the element first (not the text)
    const distanceHeader = document.getElementById('distance-header');
    
    // Now safely update its text
    distanceHeader.textContent = "Distance (Squared)";
...
document.getElementById("plot-analysis").addEventListener("click", function () {
    ...
    Swal.fire({
        ...
        imageUrl: 'images/Presentation1.jpg',
        imageWidth: 500,
        imageHeight: 300,
...
$("#button-distance").click(async function () {
    ...
    alert2('info', 'Determine the cluster for every data-point', 'Assign the data point to the cluster whose <br> centroid is closest (i.e., has the minimum euclidean distance) <br> by clicking on the <b style="color:#FF6600">CLUSTER</b> button.', enableClusterButtonData());
...
function updateSSEInputs() {
    ...
    alert1('success', '', 'Click on the <b style="color: #FF6600">NEXT</b> button to compute SSE for next value of K.');
...
function highlightCells(clusterArray) {
    ...
    for (let i = 0; i < clusterArray.length; i++) {
        ...
        dataCell.style.backgroundColor ='rgb(229 184 153)';
...
function SSEnextButton() {
    ...
    alert1('info', 'Calculate Sum of Squared Error (SSE)!', 'Enter the highlighted values <b style="color:#FF6600">(minimum distance)</b> from the table into the SSE input box.');
...
function analyseSectionClick() {
    ...
    alert1('info', 'Plot SSE against K', 'After calculating SSE for each value of K, plot a graph by clicking on <b style="color:#FF6600">PLOT</b> button.<br> Where, K is on the x-axis, and SSE is on the y-axis.')
