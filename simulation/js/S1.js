...
function addDataPoint() {
    ...
    if (xValue == '+' || xValue == '-' || xValue == '*' || xValue == '/' || yValue == '+' || yValue == '-' || yValue == '*' || yValue == '/') {
        alert1('error', 'Invalid Values!', 'Please a enter valid value.');
    ...
    if (isNaN(xValue)) {
        alert1('error', 'Missing Value!', 'Please enter a value for X(-100,100).');

        return;
    }
    if (isNaN(yValue)) {
        alert1('error', 'Missing Value!', 'Please enter value for Y(-100,100).');

        return;
    }

    if (xValue > 100 || yValue > 100 || xValue < -100 || yValue < -100) {
        alert1('error', 'Out of range!', 'Valid range is -100 to 100.');
    ...
    if ((isNaN(xValue) || (isNaN(yValue)))) {
        ...
        alert1('error', 'Invalid Input!', 'Please enter a number between -100 to 100.');
...
function tabChange() {
    ...
    document.getElementById('tab-1-text').style.padding="5px 10px";
    document.getElementById('tab-2-text').style.padding="5px 10px";
...
function promptToChangeCentroids() {
    Swal.fire({
        // title: 'Delete Rows',
        text: "Do you want to change selected centroid?",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
...
function tabChangeIntoAnalysis() {
    ...
    document.getElementById('ana').style.padding="5px 10px";
    document.getElementById('tab-1').style.backgroundColor = "#444648";
    document.getElementById('anaimg').src = 'images/ANALYSIS ICON1.png';
...
$("#button-distance").click(async function () {
    ...
    alert2('info', 'Determine the cluster for every data-point', 'Assign the data point to the cluster whose centroid is closest (i.e., has the minimum euclidean distance) by clicking on the <b style="color:#FF6600">CLUSTER</b> button.', enableClusterButtonData());
...
function reset() {
    ...
    if(kVal!=2)
    {
        document.getElementById('centroid-heading').innerHTML='INITIALIZE CENTROIDS'
        document.getElementById('label-centroid').innerHTML='Select Centroids :'
    }
...
$("#new-centroid-button").click(function () {
    ...
    setTimeout(() => {
        alert2('info', 'Find euclidean distance based on the new centroids.', 'Now, click on the <b style="color:#FF6600">DISTANCE</b> button.', enableDistanceButtonData());
            ...
            if (clusterValue == countSSEHighLightInputTag) {
                ...
                if (inputTag) {
                    ...
                    if (kVal + 1 < countSSEHighLightInputTag) {
                        ...
                        console.log("total sse highlight tag run".countSSEHighLightInputTag)
...
function analyseSectionClick() {
    ...
    alert1('info', 'Plot Silhouette Score against K', 'After calculating overall Silhouette Score for each value of K, plot a graph by clicking on <b style="color:#FF6600">PLOT</b> button.<br> Where, K is on the x-axis, and Silhouette Score is on the y-axis.')
}
...
$('#button-SEPARATION').click(async function () {
    ...
    alert1('info', '', 'Input the highlighted value from the table into the silhouette equation. Then, click on <b style="color:#FF6600">SUBMIT</b> button.');
...
function SilhouetteCalculation() {
    ...
    if (cohesionValues[Scounter] != parseFloat(parseFloat(a_i).toFixed(2))) {
        if (a_i == '') {
            ...
            alert1('error', '', 'Please enter a required value in the highlighted input box.');
            ...
            let a = document.getElementById('a_i');
            a.style.border = "2px solid red";
            alert1('error', '', 'Please enter a correct value in the highlighted input box.');
...
document
    ...
    .addEventListener("click", async function computeIsOn() {
        ...
        let eq_var=document.getElementById('eq-form');
    eq_var.style.left = "77px";
    eq_var.style.top = "";
    eq_var.style.fontSize = "16px";


        document.getElementById('sAvg').style.left = "-2px";

        $('#s-avg-cal').hide();
        document.getElementById('SSE_next').disabled = true;
        eq_var.innerHTML = ` <div class="input-container-s-avg" id="avg-s-coff">
...
function highlightCells() {
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        sCell.style.backgroundColor = "rgb(227 197 177)";
...
function updateSSEInputs() {
    ...
    let eq_res=document.getElementById('eq-form');
    eq_res.style.left = "269px";
    eq_res.style.top = "64px";
    eq_res.style.fontSize = "24px";
